...
const USE_MOCK_API = true; // Change to false when backend is available
...
export const api = {
  ...
  async updateMood(mood: number): Promise<{ mood: number }> {
    if (USE_MOCK_API) {
      await new Promise(resolve => setTimeout(resolve, 500));
      return { mood };
    }
  ...
  // Chat endpoint
  async sendChatMessage(message: string, theme: string, mood: number): Promise<ChatResponse> {
    if (USE_MOCK_API) {
  ...
  // Exercise endpoint
  async submitExerciseAttempt(exerciseId: string, startTs: string, endTs: string, duration: number): Promise<ExerciseResponse> {
    if (USE_MOCK_API) {
  ...
  // Booking endpoints
  async createBooking(counsellorId: string, slot: string, shareContext: boolean): Promise<BookingResponse> {
    if (USE_MOCK_API) {
      await new Promise(resolve => setTimeout(resolve, 1000));
      return {
  ...
  async requestMatch(studentId: string, locale: string, summary: string): Promise<MatchResponse> {
    if (USE_MOCK_API) {
      await new Promise(resolve => setTimeout(resolve, 500));
      return {
  ...
  async getAnalytics(from: string, to: string): Promise<AnalyticsResponse> {
    if (USE_MOCK_API) {
      await new Promise(resolve => setTimeout(resolve, 1000));
      return {
        dau: 123,
